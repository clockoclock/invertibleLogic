...
def main():
    ...
    # 节点对应: 0:m0(In1), 1:m1(In2), 2:m2(Out), 3:m3(Aux1), 4:m4(Aux2)
    # 注意：此处 h 数组参数与上方注释示例不同，是根据实际优化或实验需求设定的另一组参数，请根据具体物理模型选择合适参数。
    h = np.array([0, 0, 2, -3, 3], dtype=float)
    ...
    # 辅助位状态分类容器 (m3 m4 共有 4 种微观组合，顺序依次为 m3=0,m4=0 到 m3=1,m4=1，需与后续数据结构严格对应)
    ...
    for b in range(32):
        ...
        # 转换为自旋状态 s 轴 (-1, +1)
        # 物理意义说明：二进制位 m=0 映射为自旋 s=-1，m=1 映射为自旋 s=+1（即 m∈{0,1} → s∈{-1,+1}），
        # 这是量子自旋系统常用的二进制到自旋态的映射方式。
        s = 2 * m - 1
        ...
        # 纯粹的 XOR 逻辑真值表合法性检验
        # 说明：XOR 真值表要求输出 m[2] 必须等于 m[0] 与 m[1] 的异或结果（即仅当输入不同，输出为 1，否则为 0），
        # 因此 (m[0] ^ m[1]) == m[2] 用于判断当前微观状态是否满足 XOR 逻辑门的输出要求。
