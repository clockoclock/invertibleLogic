...
# Created by: WHH
# Date: 2026-05-19
# Description: 4-input AND 逻辑门（引入辅助位）模拟退火统计验证，通过三个AND门级联和辅助位连接完成设计
# 该代码实现了一个基于 Ising 模型的 4 输入 AND 逻辑门模拟器，使用了模拟退火算法来观察系统状态的演化轨迹，并通过统计分析展示了系统在不同阶段的状态分布情况。
...
# Result: 功能正常，能够正确模拟 4 输入 AND 逻辑门的行为，并且在统计分析中显示合法状态的捕获率，同时通过轨迹图展示了系统状态的演化过程。
...
# ==========================================
# 节点定义: 0:In1, 1:In2, 2:Out1_a, 3:In3, 4:In4, 5:Out2_a, 6:Out1_b, 7:Out2_b, 8:Out
h = np.array([-1.0, -1.0, 2.0, -1.0, -1.0, 2.0, -1.0, -1.0, 2.0])

# 耦合矩阵 J (对称矩阵)
J = np.zeros((9, 9))
# Gate A (In1, In2 -> Out1_a)
J[0,1] = J[1,0] = 1.0
J[0,2] = J[2,0] = -2.0
J[1,2] = J[2,1] = -2.0
# Gate B (In3, In4 -> Out2_a)
J[3,4] = J[4,3] = 1.0
J[3,5] = J[5,3] = -2.0
J[4,5] = J[5,4] = -2.0
# Gate C (Out1_b, Out2_b -> Out)
J[6,7] = J[7,6] = 1.0
J[6,8] = J[8,6] = -2.0
J[7,8] = J[8,7] = -2.0

# Copy-Link (Out1_a <-> Out2_a) 强一致性约束
J[2,6] = J[6,2] = -1.5
J[5,7] = J[7,5] = -1.5
...
def is_legal(s):
    """判断逻辑是否合法: (In1&In2==Out1_a) AND (In3&In4==Out2_a) AND (Out1_a&Out2_a==Out) AND Copy-Link约束 (Out1_a==Out1_b) AND (Out2_a==Out2_b)"""
    m = (s + 1) // 2
    cond1 = (int(m[0]) & int(m[1])) == int(m[2])
    cond2 = (int(m[3]) & int(m[4])) == int(m[5])
    cond3 = (int(m[6]) & int(m[7])) == int(m[8])
    cond4 = int(m[2]) == int(m[6])
    cond5 = int(m[5]) == int(m[7])
...
# 32个宏观逻辑状态定义与 4输入 AND 真值表合法态映射
labels = sorted([format(i, '05b') for i in range(32)])
legal_and3 = ["00000", "00010", "00100", "00110", "01000", "01010", "01100", "01110", "10000", "10010", "10100", "10110", "11000", "11010", "11100", "11111"]

print("正在进行1000次由【全系统总能量差】驱动的退火仿真与统计分析...")
for _ in range(num_runs):
    final_s = simulated_annealing_global_energy(h, J)
    # 提取其中5位宏观逻辑状态位 (In1, In2, In3, In4, Out)
    m = (final_s + 1) // 2
    state_str = "".join(map(str, m[[0, 1, 3, 4, 8]].astype(int)))
...
# --- 新增：打印详细的控制台数据统计报表 ---
print("\n" + "="*65)
print("     4 输入 AND 门 (Copy-Link拓扑) 宏观逻辑状态收敛统计")
print("="*65)
print(f"{'逻辑状态(In1234 Out)':<22} | {'收敛次数':<10} | {'收敛概率':<10} | {'状态性质'}")
print("-"*65)
...
plt.xlabel("Logic State (In1 In2 In3 In4 Out)", fontsize=12)
