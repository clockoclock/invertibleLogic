...
# Date: 2026-05-16
# Created by JCZ
# Description: 验证 2-input XOR 逻辑门的 Ising 模型参数是否正确，通过统计模拟退火的结果来评估模型的准确性。 
...
def verify_2in_xor_with_2aux_counts(num_samples=10000, T=5):
    """
    # 1. 填入你求解出的参数 (image_be8300.png)
    h = [0, 0,  3, 3, -3]  # [h0, h1, h2, h3, h4]
    ...
    for _ in range(num_samples):
        # 随机初始化 5 个节点状态为物理自旋 {-1, 1}
        s = np.random.choice([-1, 1], size=5)
        
        # ================== 【核心修改：激活模拟退火过程】 ==================
        for step in range(250):
            t = T * (1 - step / 250) + 1e-5  # 从初始 T 线性降温至接近 0
            for i in range(5):
                s_flip = s.copy()
                s_flip[i] *= -1
        ...
        # 转换为数字逻辑 0/1 格式，只记录核心逻辑关注的 [In1, In2, Out]
        logical_state = tuple((s[:3] + 1) // 2)
    ...
    print(f"{'='*60}")
    print(f"2-Input XOR 正向功能验证结果 (总样本数: {num_samples})")
    print(f"{'状态 (In1 In2 Out)':<18} | {'出现次数':<8} | {'占比':<8} | {'逻辑判定'}")
    print("-" * 60)

    total_valid = 0
    # 遍历所有 8 种可能的输入输出逻辑组合 (2^3)
...
if __name__ == "__main__":
    verify_2in_xor_with_2aux_counts()